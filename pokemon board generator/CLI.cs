...
namespace pokemon_board_generator
{
    class CLI { 
        ...
        {
            Console.WriteLine(PokemonAccess.versionString);
            Console.WriteLine(PokemonAccess.rulesString);

            DataParser parser = new DataParser();
            //Console.WriteLine(PokemonAccess.GetMethod(PokemonAccess.Pokedex[0], GameVersion.Yellow));
            foreach (Pokemon p in PokemonAccess.Pokedex)
            {
                PokemonAccess.MarkGameAccess(p);
            }

            foreach (Pokemon p in PokemonAccess.RegionalPokedex)
            {
                Console.WriteLine("#{0:D3} {1}", p.number, p.name);
            }
            Console.Read();
